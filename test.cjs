...
assert.equal(globalThis.ELIN_TRAITS.ActSword.rapid,true);
...
const caster={spell:'50500',level:10,attribute:100,enhance:0,anti:0,characterLevel:30};
assert.equal(calculate({...caster,isPC:true,isAlly:true},data).power,130);
assert.equal(calculate({...caster,isPC:false,isAlly:false},data).power,210);
assert.equal(calculate({...caster,isPC:false,isAlly:true},data).power,422);
const valor=calculate(songInput,data).statusEffects;
assert.deepEqual(valor.filter(e=>e.name.startsWith('周囲の味方')).map(e=>e.value),[6,5,6]);
assert.ok(calculate({...songInput,spell:'6754'},data).statusEffects.some(e=>e.text?.includes('歌姫反映後 +3%')));
assert.ok(calculate({...songInput,spell:'6750'},data).statusEffects.some(e=>e.text?.includes('通常の敵 22%')));
assert.ok(effects('50800').some(e=>e.text==='武器ダメージを属性ダメージに変換'));
console.log('Passed: PC/NPC allegiance power floors, song effects and supplemental descriptions.');
