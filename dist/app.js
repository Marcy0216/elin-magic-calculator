(()=>{
  ...
  function name(r){const ref=r.alias.includes('_')&&data.elements.find(e=>e.alias===r.aliasRef);return ref?ref.name_JP+'の'+r.name_JP:r.name_JP;}
  ...
  function render(){
    ...
    addFeature('連続魔法',yes(t.rapid));addFeature('長押しで反復',yes(t.repeat==='tag'?r.tag.split(',').includes('repeat'):t.repeat));addFeature('魔法強化',kind?kind==='spell'||kind==='breathe'||kind==='song'?'適用':'適用なし':'未確認');addFeature('反魔法による威力低下',kind?kind==='spell'?'あり':'なし':'未確認');addFeature('対応主能力',parent?.name_JP??'なし');addFeature('歌姫',kind==='song'?'適用':'適用なし');
    $('featureNote').textContent=t.rapid?'連続魔法フィートがある場合に追加発動の対象になります。表示ダイスは1回分です。':'適用可否は発動に必要な条件を満たした場合のものです。';
    if(!kind){showStatusEffects(r.codeEffects||[]);$('power').textContent='未確認';$('dice').textContent='未確認';$('diceNote').textContent='この種類の計算処理はまだ確認できていません。';return {power:null,dice:null};}
  ...
  const profileIds=['isPC','isAlly','characterLevel','enhance','anti','music','diva',"stat-STR","stat-END","stat-DEX","stat-PER","stat-LER","stat-WIL","stat-MAG","stat-CHA","stat-LUC","stat-SPD"];
