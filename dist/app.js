(()=>{
  ...
  function render(){
    ...
    const automatic=r.group==='ABILITY'&&$('autoAbility').checked;
    $('level').disabled=automatic;
    if(automatic){const lv=Number($('characterLevel').value);$('level').value=$('characterLevel').value.trim()===''?'':Math.trunc(10*(100+Math.trunc((lv-1)*r.lvFactor/10))/100);}
    $('levelHint').textContent=automatic?'キャラクターレベルから求めた基本値です。個別の補正を指定する場合はチェックを外してください。':'魔法・アビリティごとの補正後レベルを入力してください。';
  ...
  const profileIds=['characterLevel','enhance','anti','music','diva','rapid',"stat-STR","stat-END","stat-DEX","stat-PER","stat-LER","stat-WIL","stat-MAG","stat-CHA","stat-LUC","stat-SPD"];
